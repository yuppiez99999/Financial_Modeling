...
- **动机（顺序风险）**：T26.1 是「是否接受 ~1.7GB 重型依赖」的人工检查点。
  要是先批依赖、再在 T26.3 发现对照**不可离线复现**，那 1.7GB 就被**白批**了。
- **实测两条结构事实**（不是"数据会变"这种含糊说法）：对照用的
  `data/raw/<symbol>.csv` 是**滑动增量缓存** —— ① 只留最近 N 行（实测 400 行，
  窗口随追加滑动）；② **按日期标签对齐，而标签是错位的**（同一价格序列的
  日期整体偏 5~7 个交易日）。⇒ 同一模型输出在两天的对照会落在**不同价格序列**上。
- 交付 `src/eval/laya_frozen_replay.py`：按**位置**逐行对账金额列（判定是否同一
  价格序列）+ 日期标签**逐值**核（区间相同而内部错位最隐蔽）+ 结论词表
  `replayable` / `replayable_with_label_drift` / `snapshot_diverged` / `unverifiable`。
  CLI `python main.py laya-replay` → `reports/laya_replay.json`。
- **主读数（真实数据，只读）**：成对 3/28（其余 25 只缓存窗口已滑走）；
  价格逐行相等 ✅ 3/3；日期标签错位 ❌ 0/3（偏 5~7 个交易日）
  ⇒ **`replayable_with_label_drift`**：对照**可离线复算**，但口径**必须认冻结快照的
  日期**，不得混用缓存错位标签（否则与 `reports/*.frozen.*.json` 对不上）。
- 纪律：全部产出 `affects_gate`/`affects_signal` 恒 False；**不写数据、不改配置、不联网**；
  缺缓存/缺快照/长度不足如实 `unverifiable`。新增 `tests/test_laya_frozen_replay.py`
  17 条守卫（全离线合成）；`pytest tests` → **1516 passed / 4 skipped**。
- **边界**：本模块只回答「对照能不能离线复算」，**不**回答「Laya 有没有用」
  （那仍须 T26.1 批依赖 + 真实权重的 T26.3 对照）；T26.1/T26.5 仍 `pending`，NPC 不代签。
- 详见 `00_kickoff/s26_laya_decision_conclusion.md` §3.1。
