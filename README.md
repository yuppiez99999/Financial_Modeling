...
| `monitor` | 生成模型监控报表（审计命中率 / 漂移 / 数据源 / 模型产物 / 实时流 / IC 趋势 / 分池 / 多周期扫描） |
...
> **S10 落地的不是这次变更，而是支撑这次决策所需要的能力**。

### 17.1 它解决什么问题

「把门禁周期从 20 日改成 40 日」这个决定，在此之前只能靠 §16.5 里**一次性**跑出的
表格来支撑。一次性结果有三个问题：

1. **不可复算**：换个数据区间/标的池，没人能快速回答"结论还成立吗"；
2. **不可审计**：说不清"扫了哪几个周期、用的是不是同一套阈值"；
3. **容易被误用**：看到"40 日达标"就直接当"信号已可用"，跳过人工决策。

S10 把它变成一条命令：对一组候选周期各自独立跑**同一套** walk-forward 口径，
输出可复算、可审计、**默认不改门禁**的证据。

```bash
python main.py horizon-scan              # 候选 5/10/20/40/60 日，含分池
python main.py horizon-scan --no-pools   # 只扫整池，更快
python main.py horizon-scan --days 5,20,40   # 自定义候选周期
```

| 产物 | 位置 |
|------|------|
| 扫描报告 | `reports/horizon_scan.json` |
| 监控报表 | 「多周期口径探索（S10）」章节 |
| 日报 / 周报 | 「📐 多周期口径探索（S10）」章节 |
| API（只读） | `GET /api/v1/strategy/horizon-scan` |

### 17.2 关键设计：为什么不自动选周期

本模块**刻意不做**"挑一条能过线的周期然后宣布解锁"。原因：

> 在 5 个候选周期里挑最好看的那个当结论，是典型的**多重比较**。
> 候选越多，"至少有一个偶然过线"的概率越高 —— 那不是信号变强，
> 是**选择偏差**让指标失真。
...
| 约束 | 做法 | 理由 |
|------|------|------|
| **不改现行门禁** | `data.prediction_horizons` 一个字不动，`affects_gate` 恒为 `False` | 扫描是证据，不是解锁通道 |
| **不自动切换** | 达标周期只写进结论叙述，**没有任何代码路径**据此调用 `gated` | 口径变更须人工决策 + 重做泄漏审查 |
| **不硬凑** | 周期长于数据长度 → `available=false` + 原因，**绝不跑半截窗口** | 半截窗口算出的 IC 没有含义 |
| **不猜** | 序列构造失败 / 样本不足 → 如实标注不可用，不填默认值 | 沉默的默认值会伪装成"结论" |
| **不看好看的池** | 分池结果同样只报告，不参与放行判定 | 与 §16.3 分池边界同构 |

### 17.3 真实 26 标的池实测（本机缓存行情，非合成数据）

`python main.py horizon-scan --no-pools`，腾讯前复权日K · walk-forward 3 折 · 无前视：

| 预测周期 | 现行口径 | 样本 | IC | 命中率 | 结论 |
|---------|---------|------|-----|--------|------|
| 5 日 | ✅ | 12418 | −0.0015 | 48.80% | ❌ |
| 10 日 | ✅ | 12340 | −0.0110 | 47.81% | ❌ |
| 20 日 | ✅ | 12184 | +0.0365 | 51.25% | ❌ |
| **40 日** | — | 11872 | **+0.0913** | **54.29%** | ✅ |
| **60 日** | — | 11560 | **+0.1239** | **58.37%** | ✅ |

**结论**：与 §16.5 独立复现一致 —— 现行三周期全部未过，40/60 日达标，
且随周期拉长单调走强（IC −0.0015 → +0.1239，命中率 48.80% → 58.37%）。
**这次是用一条命令复算出来的，不是一次性实验。**
...
```bash
python -m pytest tests/test_roadmap_s10.py -q   # 36 passed
python -m pytest tests/ -q                      # 462 passed, 2 skipped
```

新增 `tests/test_roadmap_s10.py`（36 项）：候选周期去重/升序/非法值丢弃不崩、
未配置时回落默认、现行口径标注只对配置周期为真；样本不足 fail-close 且给原因、
纯噪声永不达标、强信号在长周期达标；**`affects_gate` 恒为 False**、
对照文案须声明"不是自动解锁"、现行已达标时不建议切换、全不达标时说"换周期无用"；
覆盖度对超长周期与空/非法输入的处理；分池独立报告且不改整池结论；
落盘/读取/缺失/损坏四分支、报表行不塞原始序列；
监控报表 / 日报的章节渲染与缺失分支、API 正常与缺失分支；
配置段存在且**现行周期未被改动**；CLI 契约与候选解析。

### 17.6 合规边界

- 扫描**不改变** `strategy_gate` 判定（`affects_gate=false`），`enforce_trading` 行为不变；
