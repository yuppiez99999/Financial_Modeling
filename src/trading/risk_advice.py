"""Q4 路线：智能风控建议（自动生成止损/止盈建议）。

路线图（`SALES_PLAN.md` §8.2）Q4：
  > 智能风控模块，自动生成止损止盈建议

## 与 `RiskManager` 的分工（不要重复造轮子）

`src/trading/risk.py` 解决的是「**这笔交易下多少钱**」——固定分数/凯利算仓位，
止损止盈是**固定百分比**推算出来的附属字段，`price * (1 - 0.03)`。
它不回答 Q4 要问的两个问题：

1. **比例定多少**：3% 对 A 股 ETF 偏松、对高波动个股偏紧。比例应当由标的自身的
   波动结构决定，而不是全池共用同一个常数。
2. **价位定在哪**：整数百分比止损几乎必然落在噪音里 —— 要么被日内正常波动打掉，
   要么宽到失去意义。价位应当参考真实支撑阻力（近期低点/均线/ATR 缓冲）。

本模块只做这两件事，输出**建议**（suggestion），不改动 `RiskManager`/`OrderGenerator`
的任何行为，也不产出下单指令。

## 合规与安全边界（必须成立，否则本模块就是有害的）

- **门禁未放行时不产出价位建议**：`strategy_gate` 为 `readonly` 时信号只作观测，
  给一个未过门禁的信号配"精准止损价"会让下游误以为可用于交易。
  此时返回 `status=withheld`，只保留仓位侧信息，`stop_price/take_price` 为 `None`。
- **不构成投资建议**：输出带 `disclaimer` 与 `not_trade_instruction` 标记；
  所有结果字段命名统一为 `suggested_*`，不出现 `order` / `execute` 语义。
- **信息不足时拒绝给数**：无行情、价格非法、样本不足 → 如实标注
  （`available=False` / `reason`），绝不用默认比例兜底算一个"看起来能用"的价。
- **无未来函数**：只读已落盘的日K（T 及更早），不触网、不重算当日盘中数据。
"""
...
class RiskAdvisor:
    ...
    def gate_state(self) -> str:
        ...
        try:
            ...
            logger.warning("[risk_advice] 读取门禁状态失败: %s", e)
            return "unknown"
    ...
    def _true_range(df: pd.DataFrame) -> pd.Series:
        """真实波幅 TR = max(H-L, |H-PC|, |L-PC|)，首行无前收 → 退化为 H-L。"""
    ...
    def atr(self, df: pd.DataFrame) -> Optional[float]:
        """平均真实波幅（Wilder 平滑）。样本不足返回 None，不猜。"""
        if df is None or len(df) < max(self.atr_window, 2):
            return None
        tr = self._true_range(df)
        if tr.dropna().empty:
            return None
        return float(tr.ewm(alpha=1.0 / self.atr_window, adjust=False).mean().iloc[-1])

    def _ma(self, df: pd.DataFrame, window: int) -> Optional[float]:
        """观测用均线（含最后一根）：ATR 窗口期均线是价格所在位置的自然参照。"""
