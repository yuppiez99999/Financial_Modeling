...
| `edge-check` | **基准相对决策增量**（信号组合 vs 全池等权的净超额 + 扣成本 + 随机子集对照 + **状态分层净超额**：三分 `bull/range/bear` + **趋势/盘整二分 `binary`**；`--holding-horizon` / `--cost-level` / `--random-controls` / `--edge-thr` / `--no-regime-breakdown` / `--regime-refit-every` / `--regime-window`） |
