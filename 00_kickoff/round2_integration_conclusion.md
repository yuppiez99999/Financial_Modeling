...
2. **人工检查点不代签**：T16.4 / T17.4 / T18.4 / T19.4 / T20.4 已由用户在
   Issue #40 确认（`confirmed`，带签署字段）；**`completed` 依然禁止**（= 代签）；
...
## 五、人工确认与收官（2026-09-12 追加）

**用户指令**（Issue #40）：「这11项确认 并继续按排期计划开发」。

**确认对象 = 本文件给出的「现状默认值」**，不是改配置的授权。11 条结论：

| 检查点 | 确认结论 | 决策 |
|:---|:---|:---:|
| T15.3 | 门禁维持只读（`report_only`），**本次不解锁** | `defer` |
| T16.4 | 不切口径：概率距离口径作生产刻度 | `defer` |
| T17.4 | 不引入过拟合概率下限这条腿 | `defer` |
| T18.4 | 不改配置：不进门禁/风控，也不作生产特征 | `moot_by_convention` |
| T12.3 | 标签主线不切换 | `defer` |
| T13.4 | 明确**不纳入** Alpha158 因子 | `reject` |
| T14.3 | 按现状定稿（akshare 优先；期货/外汇进主线） | `keep` |
| T11.2 | 成本三档定稿，禁止配置覆盖 | `keep` |
| T19.4 | 校准层暂不进主推理链路 | `defer` |
| T20.1 | 候选**不采纳**（0/3 满足准入） | `reject` |
| T20.4 | LLM 投研链路**整阶段取消** | `cancel` |

**由此产生的三个可审计后果**

1. **S11~S20 十个阶段收官为 `completed`**，每阶段 `closing` 记录
   「自动部分已完成 + 人工检查点已确认 + `affects_gate=false`」；
2. **`strategy_gate` 依然逐字段零变更** —— 确认的是现状，不是变更；
3. **T15.3 / T18.4 / T19.4 的「耦合」不再构成阻塞**：三者本次**都没有变更**，
   阈值与概率语义维持原状，因此不存在语义漂移问题（§14.2 的提醒仍然有效，
   适用于**未来**任何一次要动这三项之一的提案）。

**守卫升级**：`tests/test_manual_checkpoints_confirmation.py`（28 例）钉死
「确认必须可追溯（签署人/时间/决策/范围/不在范围/指令原文/证据）」
「确认不得顺带改 `strategy_gate`」「人工检查点永远不得是 `completed`」
「阶段 `completed` 必须有确认字段背书」。
